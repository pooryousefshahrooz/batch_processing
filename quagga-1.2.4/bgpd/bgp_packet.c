...
{
  if(policy_checking_flag==0) /* batch processing code: we check if the policy_checking_flag is on or off. If it is off, we can not send packets.*/
    return NULL;
  struct stream *s;
  struct stream *snlri;
...
bgp_write (struct thread *thread)
{
  ...
  */
  ...
  /* batch processing code: here we first call bgp_read_all function 
  to check and read all update packets in the incomming scokets for all peers */
  bgp_read_all();
  ...
  /* batch processing code: we check if the flag for checking export policies has been set on or not */
...
/* batch processing codes: read one packet on the incomming socket of all peers and set the export policy checking off if it read at least one packet*/
